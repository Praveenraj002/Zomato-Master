...
const MobileTabs = () => {
    ...
    return <>
    <div className="lg:hidden  bg-white shadow-lg p-3 md:justify-evenly fixed bottom-0 z-10 w-full flex items-center justify-between text-gray-500 border">
...
const {type} = useParams();
    return <>
    <div className="hidden lg:flex gap-14 container px-20 my-10 mx-auto">
        ...
        <div className={classNames("flex items-center gap-3 pb-2 transition duration-700 ease-in-out", {"border-b-2 border-zomato-400" :type === items.id, })}>
            ...
            <h3 className={type=== items.id ? "text-xl text-zomato-400 font-semibold" : "text-xl text-gray-700 font-semibold"}>{items.name}</h3>
        </div></Link>) )}
    </div>
    </>
};
